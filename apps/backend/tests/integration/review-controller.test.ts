...
test("POST /review/:review_id/comment creates a comment for authenticated user", async () => {
  const db_client: Prisma.TransactionClient = iocContainer.get("DB_CLIENT");
  ...
  const res = await request(app)
    .post(`/review/${review.id}/comment`)
    .send({ desc: "Test Comment" })
    .set("Authorization", `Bearer ${access_token}`);

  expect(res.statusCode).toBe(204);

  const comment = await db_client.comment.findFirst({
    where: { AND: [{ review_id: review.id }, { profile_id: mock_profile.id }] },
  });
