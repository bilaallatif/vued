...
test("DELETE /comment/:comment_id returns error for removing comment of different user", async () => {
  const db_client: Prisma.TransactionClient = iocContainer.get("DB_CLIENT");
  ...
  // Generate access token for user
  const access_token = sign({ userId: mock_different_user.id }, "test", {
    expiresIn: "60s",
  });
  ...
  expect(orm_comment).not.toBeNull();
});
